<?php
/*
Plugin Name: k0d1r Auto Blog Writer
...
function create_random_post() {
    ...
    // Gönderi başarıyla oluşturulduysa, e-posta bildirimi gönder
    $email_subject = "Yeni Blog Gönderisi Oluşturuldu";
    $email_message = "Yeni bir blog gönderisi oluşturuldu: '" . $random_topic . "'";
    wp_mail(get_option('admin_email'), $email_subject, $email_message);
}

// Zamanlanmış bir görev oluştur
function schedule_post_creation() {
    if (!wp_next_scheduled('schedule_post_creation')) {
        wp_schedule_event(time(), 'every_minute', 'schedule_post_creation');
    }
}

add_action('wp_loaded', 'schedule_post_creation');
add_action('schedule_post_creation', 'create_random_post');

// Eklenti etkinleştirildiğinde zamanlanmış bir görev oluştur
function auto_blog_writer_activate() {
    // Zamanlanmış görevi ekler
    if (!wp_next_scheduled('schedule_post_creation')) {
        wp_schedule_event(time(), 'every_minute', 'schedule_post_creation');
    }
}
register_activation_hook(__FILE__, 'auto_blog_writer_activate');

// Eklenti devre dışı bırakıldığında zamanlanmış görevi kaldır
function auto_blog_writer_deactivate() {
    $timestamp = wp_next_scheduled('schedule_post_creation');
    wp_unschedule_event($timestamp, 'schedule_post_creation');
}
register_deactivation_hook(__FILE__, 'auto_blog_writer_deactivate');
?>